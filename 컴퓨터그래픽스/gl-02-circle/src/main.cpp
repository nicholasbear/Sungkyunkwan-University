...
static const uint	MIN_CIRCLE = 20;
static const uint	MAX_CIRCLE = 512;
uint				NUM_CIRCLE = 30;
...
int Circle_Collision() {
	for (uint i = 0; i < circles.size(); i++) {
		for (uint j = i + 1; j < circles.size(); j++) {
			float dist = sqrt((circles[i].center.x - circles[j].center.x) * (circles[i].center.x - circles[j].center.x) + (circles[i].center.y - circles[j].center.y) * (circles[i].center.y - circles[j].center.y));
			if (dist <= circles[i].radius + circles[j].radius) {//충돌
				return 1;
			}
		}
	}
...
int Wall_Collision() {
	for (auto& c : circles) {
		if (c.center.y + c.radius >= 1 || c.center.y - c.radius <= -1) {
			return 1;
		}
		if (c.center.x + c.radius >= 16 / float(9) || c.center.x - c.radius <= -16 / float(9)) {
			return 1;
		}
	}
	return 0;
...
void update()
{
	...
	void update_circle(); // forward declaration
	if(b) update_circle(); 
}
...
void render()
{
	...
	for (auto& c : circles) {
		c.update(t, prev_t, circles);
	}

	while (Wall_Collision() || Circle_Collision()) {
		//벽 충돌부터 해결
		for (auto& c : circles) {
			if (c.center.y + c.radius >= 1 || c.center.y - c.radius <= -1) {
...
void update_circle()
{
	...
	if (n > NUM_CIRCLE) {//더할때
		...
		NUM_CIRCLE = n;
		printf("> NUM_TESS = % -4d\r", NUM_CIRCLE);
	}
	else {//뺄때
		circles.pop_back();
		NUM_CIRCLE = n;
