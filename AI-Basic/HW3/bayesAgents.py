...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    both_top = {Y_POS_VAR: BOTH_TOP_VAL}
    both_bottom = {Y_POS_VAR: BOTH_BOTTOM_VAL}
    left_top = {Y_POS_VAR: LEFT_TOP_VAL}
    left_bottom = {Y_POS_VAR: LEFT_BOTTOM_VAL}

    yFactor.setProbability(both_top, PROB_BOTH_TOP)
    yFactor.setProbability(both_bottom, PROB_BOTH_BOTTOM)
    yFactor.setProbability(left_top , PROB_ONLY_LEFT_TOP)
    yFactor.setProbability(left_bottom, PROB_ONLY_LEFT_BOTTOM)
    "*** END YOUR CODE HERE ***"
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    infer = inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    res = 0
    prob = 0
    for assignment in infer.getAllPossibleAssignmentDicts():
        temp = infer.getProbability(assignment)
        if temp > prob:
            prob = temp
            res = assignment
    return res
